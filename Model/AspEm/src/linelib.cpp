...
void line_node::init(char *file_name, std::vector<int> types, int vector_dim)
{
   ...
   int node_type;
	...
	while (1)
	{
		if (fscanf(fi, "%s %d", word, &node_type) != 2) break;
     if (std::find(types.begin(), types.end(), node_type)!=types.end()) {         
         add_node(word);
     }
	}
	fclose(fi);
...
{ 
	...
	int u, v, tp, type1, type2;
	double w;
	hin_nb curnb;    
	while (fscanf(fi, "%s %d %s %d %lf %d", word1, &type1, word2, &type2, &w, &tp) == 6)
	{
		...
		u = node_v->search(word1);
		v = node_u->search(word2);        
		if (u != -1 && v != -1)
		{
			curnb.nb_id = u;
			curnb.eg_tp = tp;
			curnb.eg_wei = w;
			hin[v].push_back(curnb);
			hin_size++;
		}
      }
	}
